...
public class DrillTestJdbc implements DrillTest {
  ...
  private void executeQuery(String query) throws IOException, SQLException {
    ...
    try {
      ...
      //LOG.info("qfile Exists n contains plan : "+fileExists+" , "+getExpectedFile().contains(".plan"));
      while (resultSet.next()) {
        List<Object> values = Utils.getRowValues(resultSet);
        ColumnList columnList = new ColumnList(columnTypes, values);
        
        //if(!fileExists && getExpectedFile().contains(".plan")){
	writer.write(columnList + "\n");
      }
      if(!fileExists && getExpectedFile().contains(".plan")){
        //LOG.info("afile Exists n contains plan : "+fileExists+" , "+getExpectedFile().contains(".plan"));
