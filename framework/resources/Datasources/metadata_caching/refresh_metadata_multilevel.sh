...
then
  ${DRILL_HOME}/bin/sqlline -u "jdbc:drill:schema=dfs.$1;drillbit=${DRILL_STORAGE_PLUGIN_SERVER};auth=maprsasl"  --run=${DRILL_TEST_DATA_DIR}/Datasources/metadata_caching/refresh_metadata_multilevel.ddl
elif [ -z "$PASSWORD" ]
then
