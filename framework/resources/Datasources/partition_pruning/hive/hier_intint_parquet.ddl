...
DROP TABLE IF EXISTS lineitem_parquet_partitioned_hive_hier_intint;
CREATE TABLE IF NOT EXISTS lineitem_parquet_partitioned_hive_hier_intint (
    l_orderkey INT,
    l_partkey INT,
    l_suppkey INT,
    l_linenumber INT,
...
PARTITIONED BY (year int, month int)
ROW FORMAT DELIMITED FIELDS TERMINATED BY "|"
STORED AS PARQUET LOCATION "/drill/testdata/partition_pruning/hive/parquet/dynamic_lineitem_hierarchical_intint";

INSERT OVERWRITE TABLE lineitem_parquet_partitioned_hive_hier_intint partition (year, month)
SELECT
    l_orderkey,
    l_partkey,
    l_suppkey,
    ...
    year,
    month
FROM DEFAULT.lineitem_parquet_partitioned_hive_hier_intint;
