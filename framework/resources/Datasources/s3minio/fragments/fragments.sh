...
then
  ${DRILL_HOME}/bin/sqlline -u "jdbc:drill:schema=s3minio.tmp;drillbit=${DRILL_STORAGE_PLUGIN_SERVER};auth=${AUTH_MECHANISM}"  --run=${DRILL_TEST_DATA_DIR}/Datasources/s3minio/fragments/fragments.ddl
elif [ -z "$PASSWORD" ]
then
