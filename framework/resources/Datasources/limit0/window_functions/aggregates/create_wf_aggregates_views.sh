...
then
  ${DRILL_HOME}/bin/sqlline -u "jdbc:drill:schema=dfs.window_functions;drillbit=${DRILL_STORAGE_PLUGIN_SERVER};auth=maprsasl"  --run=${DRILL_TEST_DATA_DIR}/Datasources/limit0/window_functions/aggregates/create_wf_aggregates_views.sql
elif [ -z "$PASSWORD" ]
then
