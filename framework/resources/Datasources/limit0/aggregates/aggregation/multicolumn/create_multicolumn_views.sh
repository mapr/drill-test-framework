...
then
  ${DRILL_HOME}/bin/sqlline -u "jdbc:drill:schema=dfs.aggregation;drillbit=${DRILL_STORAGE_PLUGIN_SERVER};auth=maprsasl"  --run=${DRILL_TEST_DATA_DIR}/Datasources/limit0/aggregates/aggregation/multicolumn/create_multicolumn_views.sql
elif [ -z "$PASSWORD" ]
then
