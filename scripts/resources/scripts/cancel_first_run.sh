...
echo "Test cancel_first_run.sh started" | tee -a results/result.log
#------------------------------------------------------------------------ 

export LOG_FILE=logs/cancel_first_run.log
...
${DRILL_HOME}/bin/sqlline -u "jdbc:drill:zk=${ZOOKEEPER_HOST}:5181" -e "select manytests('test', ' udf') from (values(1));" >> $LOG_FILE 2>>${LOG_FILE} &
...
if [[ $QUERY_RESULT == *"test udf"* ]]; then
  echo "PASS cancel_first_run.sh" | tee -a $RESULT_FILE
else
  echo "-----------------------------------------------"  >> $FAILS_FILE 2>> $FAILS_FILE
  echo "FAIL cancel_first_run.sh" | tee -a $RESULT_FILE
  echo "FAILED: cancel_first_run.sh" >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
