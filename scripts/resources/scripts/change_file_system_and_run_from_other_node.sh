...
echo "Test change_file_system_and_run_from_other_node.sh started" | tee -a results/result.log
#------------------------------------------------------------------------ 

export LOG_FILE=logs/change_file_system_and_run_from_other_node.log
IFS=',' read -ra ADDR <<< "$DRILLBITS"

if [ $numDrills -lt 2 ]; then
  echo "IGNORE change_file_system_and_run_from_other_node.sh" | tee -a $RESULT_FILE
...
if [[ $QUERY_TMP_RESULT != *"true"*"The following UDFs in jar many_tests-1.0.jar have been registered"* ]]; then
  ...
  echo "FAIL change_file_system_and_run_from_other_node.sh" | tee -a $RESULT_FILE
  echo "FAILED: change_file_system_and_run_from_other_node.sh" >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
...
if [ ${ADDR[0]} != `hostname -f` ] ; then
  node=${ADDR[0]}
else
  node=${ADDR[1]}
fi

QUERY_RESULT=$(${DRILL_HOME}/bin/sqlline -u "jdbc:drill:drillbit=$node:31010" -e "select manytests('test', ' udf') from (values(1));" 2>&1)
echo "${QUERY_RESULT/: /\n}" >> $LOG_FILE 2>> $LOG_FILE

if [[ $QUERY_RESULT == *"SYSTEM ERROR"*"No match found for function signature manytests"* ]]; then
  echo "PASS change_file_system_and_run_from_other_node.sh" | tee -a $RESULT_FILE
else
  echo "-----------------------------------------------"  >> $FAILS_FILE 2>> $FAILS_FILE
  echo "FAIL change_file_system_and_run_from_other_node.sh" | tee -a $RESULT_FILE
  echo "FAILED: change_file_system_and_run_from_other_node.sh" >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
  echo "EXPECTED RESULT: " >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
  echo "SYSTEM ERROR: SqlValidatorException: No match found for function signature manytests" >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
  echo "ACTUAL RESULT: " >> $FAILS_FILE 2>> $FAILS_FILE
  echo "${QUERY_RESULT/: /\n}" >> $FAILS_FILE 2>> $FAILS_FILE
fi
