...
echo "Test register_with_less_drillbits started" | tee -a results/result.log
#------------------------------------------------------------------------ 

export LOG_FILE=logs/register_with_less_drillbits.log
IFS=',' read -ra ADDR <<< "$DRILLBITS"

if [ $numDrills -lt 2 ]; then
  echo "IGNORE register_with_less_drillbits" | tee -a $RESULT_FILE
...
if [[ $QUERY_RESULT == *"2450815"*"1"*"3"*"494"*"24508151"* ]]; then
  echo "PASS register_with_less_drillbits.sh" | tee -a $RESULT_FILE
else
  echo "-----------------------------------------------"  >> $FAILS_FILE 2>> $FAILS_FILE
  echo "FAIL register_with_less_drillbits.sh" | tee -a $RESULT_FILE
  echo "FAILED: register_with_less_drillbits.sh" >>  $LOG_FILE >> $FAILS_FILE 2>> $LOG_FILE 2>> $FAILS_FILE
